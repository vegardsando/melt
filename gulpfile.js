...
gulp.task("images", function(done) {
	...
	for (var key in IMAGES) {
		gulp.src(IMAGES[key])
			.pipe(plugins.plumber({ errorHandler: onError }))
            .pipe(plugins.imagemin({ optimizationLevel: 7, progressive: true, interlaced: true, svgoPlugins: [{removeViewBox: false}] }))
...
ncu.run({
	...
	// jsonUpgraded: true
		...
		if (size) {
			...
			gutil.log(colors.blue("All dependencies are upgradeed :)"));
		}
});
